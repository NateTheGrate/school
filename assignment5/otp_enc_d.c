...
void print_cipher(int* ciphertext, int length){
    ...
    for(int i = 0; i < length; i++){
        // format characters to ascii
        printf("%c", ciphertext[i] + 64);
    }
    printf("\n");
}
...
void encrypt(char* plaintext, char* key, int* ciphertext, int length){

    for(int i = 0; i < strlen(plaintext); i++){

        // change ' ' to '@' to handle spaces
        char temp1 = plaintext[i];
        if(temp1 == ' '){
            temp1 = '@';
        }
        ...
        int sum = ( (temp1 - 64) + (temp2 - 64));
        while(sum > 0){
            sum -= 27;
        }

        ciphertext[i] = modulo(sum, 27);
    ...
    while(1){
        ...
        switch(spawnPid){
            ...
            case 0:
                ...
                if(strncmp(init_msg, "enc", 3) != 0){
                    send_error(establishedConnectionFD, "SERVER: Error: client not otp_enc\n");
                    exit(1);
                }

                // send back identifying info
                snd(establishedConnectionFD, "enc");
